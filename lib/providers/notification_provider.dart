...
class NotificationProvider extends ChangeNotifier {
  ...
  List<AppNotification> get purchaseNotifications =>
      _notifications
          .where((n) => n.category == NotificationCategory.purchase)
          .toList();
  List<AppNotification> get sellerNotifications =>
      _notifications
          .where((n) => n.category == NotificationCategory.seller)
          .toList();

  // Get unread count
  int get unreadCount => _notifications.where((n) => !n.isRead).length;
  int get unreadPurchaseCount =>
      purchaseNotifications.where((n) => !n.isRead).length;
  ...
  void _generateOrderNotifications(Order order) {
    ...
    _addNotification(
      AppNotification(
        ...
        message: 'Order ${order.shoe.name} successfully confirmed',
    ...
    _addNotification(
      AppNotification(
        ...
        message: 'Payment for ${order.shoe.name} has been received',
        timestamp: order.orderDate.add(
          Duration(minutes: 5 + _random.nextInt(30)),
        ),
        orderId: order.id,
        imageUrl: order.shoe.firstPict,
        actionRoute: '/buying',
    ...
    if (order.status == OrderStatus.active) {
      _addNotification(
        AppNotification(
          ...
          category: NotificationCategory.purchase,
          title: 'Order Shipped',
          message: '${order.shoe.name} is on the way',
          timestamp: order.orderDate.add(
            Duration(
    ...
    if (order.status == OrderStatus.cancelled) {
      _addNotification(
        AppNotification(
          ...
          timestamp: order.orderDate.add(
            Duration(hours: 1 + _random.nextInt(6)),
          ),
          orderId: order.id,
          imageUrl: order.shoe.firstPict,
          actionRoute: '/buying',
  ...
  void _addNotification(AppNotification notification) {
    ...
    if (existingIndex == -1) {
      ...
      _notifications[existingIndex] = notification.copyWith(
  ...
  void generateSampleNotifications() {
    ...
    final sampleNotifications = [
      AppNotification(
        ...
        category: NotificationCategory.purchase,
        title: 'Order Confirmed',
        message: 'Nike Air Force 1 order successfully confirmed',
      ...
      AppNotification(
        ...
        category: NotificationCategory.purchase,
        title: 'Order Shipped',
        message: 'Adidas Ultraboost 22 is on the way',
        timestamp: now.subtract(Duration(hours: 2)),
        imageUrl:
            'assets/brand-products/Adidas/Adidas Ultraboost 22 Cloud White Core Black - 1.png',
      ),
      ...
      AppNotification(
        ...
        imageUrl:
            'assets/brand-products/New Balance/New Balance 574 Gray White - 1.png',
      ),
      ...
      AppNotification(
        ...
        timestamp: now.subtract(Duration(days: 1)),
        imageUrl:
            'assets/brand-products/Puma/PUMA RS-X Mix White Black - 1.png',
      ),
      ...
      AppNotification(
        ...
        timestamp: now.subtract(Duration(days: 2)),
        imageUrl:
            'assets/brand-products/Converse/Converse Chuck Taylor 70s Black - 1.png',
      ),
    ];
